...
def convert_sparse(a):
    ...
    # count
    output[2] = (a & (2 ** 12 - 1)).astype(np.uint32)
    return output
...
def compute_g2(G2, det, output_dict=None, rot_img=False):
    mask = det.mask
    qmap_sta = det.sqmap * mask
    qmap_dyn = det.dqmap * mask
    if rot_img:
    ...
    for ii in range(ql_sta_dim):
        idx_corr = (qmap_sta == ii+1).flatten()
        G2q[..., ii] = np.mean(G2[..., idx_corr], axis=2)

    Isymmq = np.multiply(G2q[:, 1], G2q[:, 2])
    Isymmq = (np.where(Isymmq != 0, Isymmq, 10000))

    for ii in range(ql_dyn_dim):
        idx_corr = (qmap_dyn == ii+1).flatten()
